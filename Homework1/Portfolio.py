...
class Portfolio():
  ...
  def withdrawCash(self, cash):
    if self.cash >= cash:
       self.cash -= cash
       transaction_statement = "{:.2f}".format(cash) + "$ were removed the portfolio."
       self.transaction_history.append(transaction_statement)
       print(transaction_statement)
    else:
  ...
  def buyStock(self, number_of_shares, stock):
    ...
    if self.cash >= price:
       ...
       tranasaction_statement = "{:.0f}".format(number_of_shares) + " shares of " + stock.symbol + " stock were bought."
       self.transaction_history.append(tranasaction_statement)
       print(tranasaction_statement)
  ...
  def sellStock(self, symbol, number_of_shares):
    ...
    for key in self.stocks:
      ...
      if key.symbol == symbol and self.stocks[key] >= number_of_shares:
         ...
         transaction_statement = "{:.0f}".format(number_of_shares) + " shares of " + stock.symbol + " stock were sold."
         self.transaction_history.append(transaction_statement)
         print(transaction_statement)
  ...
  def buyMutualFund(self, number_of_shares, mutualFund):
    ...
    if self.cash >= number_of_shares:
       ...
       self.cash -= price
       transaction_statement = "{:.2f}".format(number_of_shares) + " shares of " + mutualFund.symbol + " mutual funds were bought."
       self.transaction_history.append(transaction_statement)
       print(transaction_statement)
    else:
  ...
  def sellMutualFund(self, symbol, number_of_shares):
    ...
    for key in self.mutualFunds:
      if key.symbol == symbol and self.mutualFunds[key] >= number_of_shares:
         ...
         transaction_statement = "{:.2f}".format(number_of_shares) + " shares of " + symbol + " mutual funds were sold."
         self.transaction_history.append(transaction_statement)
         print(transaction_statement)
  ...
  def __str__(self):
    return "\nBalance: {}. \nStocks: {}\nMutual Funds: {}\n".format("{:.2f}".format(self.cash), self.stocks, self.mutualFunds)
